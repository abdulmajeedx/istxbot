...
async def cmd_stats(message: Message):
    ...
    if PRIORITY_QUEUE_ENABLED:
        ...
        if queue_manager:
            ...
            stats_text += "📥 *التحميلات*\n"
            stats_text += f"  • الإجمالي: {status['total_downloads']}\n"
            stats_text += f"  • ✅ ناجحة: {status['successful_downloads']}\n"
            stats_text += f"  • ❌ فاشلة: {status['failed_downloads']}\n"
            stats_text += f"  • � نشطة: {status['active_downloads']}\n"
            stats_text += f"  • ⏳ في الانتظار: {status['queue_size']}\n"
            stats_text += f"  • ⏱️ متوسط الوقت: {status['average_time']}\n\n"
            
            stats_text += "👥 *المستخدمون*\n"
            stats_text += f"  • 🟢 المتواجدون: {status['online_users']}\n"
            stats_text += f"  • 📅 هذا الشهر: {status['monthly_users']}\n"
            stats_text += f"  • 📊 الإجمالي: {queue_manager.stats.total_users}\n\n"
            
            stats_text += "🎯 *تحميلات المستويات*\n"
            stats_text += f"  • 👑 VIP: {queue_manager.stats.vip_downloads}\n"
            stats_text += f"  • ⭐ Premium: {queue_manager.stats.premium_downloads}\n"
            stats_text += f"  • 🆓 Free: {queue_manager.stats.free_downloads}\n\n"
    else:
        stats_text = f"� *إحصائيات البوت*\n\n{divider}\n\n"
        stats_text += "📥 التحميلات: غير متوفرة\n\n"
    
    banned_users = load_banned_users()
    stats_text += f"🚫 *المحظورون:* {len(banned_users)}\n\n"
...
async def cmd_admin(message: Message):
    ...
    if PRIORITY_QUEUE_ENABLED:
        sections.append(("🎯 نظام الأولويات والنقاط", [
            "`/settier <id> <tier>` - تغيير المستوى",
            "   المستويات: `VIP` `PREMIUM` `FREE`",
            "`/addpoints <id> <pts>` - إضافة نقاط",
            "`/resetpoints <id>` - تصفير النقاط",
