...
class BotRunner:
    ...
    async def setup(self):
        try:
            ...
            self._reload_task = asyncio.create_task(self._check_reload_signal())
    ...
    async def _check_reload_signal(self):
        """Background task: check for WordPress reload signal"""
            ...
            try:
                if os.path.exists(signal_file):
                    ...
                    if mtime > self._last_reload_check:
                        ...
                        new_admin_ids = get_wp_option('tk_bot_admin_ids', '')
                        
                        if new_token and new_token != settings.BOT_TOKEN:
                            logger.info(f"🔄 Bot token updated from WordPress")
                            settings.BOT_TOKEN = new_token
                        
                        if new_admin_ids:
                            try:
                                ids = [int(x.strip()) for x in new_admin_ids.split('\n') if x.strip()]
                                if ids and ids[0] != settings.ADMIN_ID:
                                    settings.ADMIN_ID = ids[0]
                                    logger.info(f"🔄 Admin ID updated to: {ids[0]}")
                            except:
                                pass
                        
                        logger.info("✅ Settings auto-reloaded from WordPress")
            except Exception as e:
